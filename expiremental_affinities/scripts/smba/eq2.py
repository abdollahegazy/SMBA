...
def main():
    ...
    for eq_dir in tqdm(all_dirs, desc="Checking"):
        # eq must be done
        if not (eq_dir / "system_eq000.coor").exists():
            continue

        # already running?
        script = eq_dir / "eq2.sh"
        name = get_job_name(script)
        if name and name in running:
            continue

        # already done?
        if (eq_dir / "system_eq001.coor").exists():
            continue
    ...
    with tqdm(total=len(remaining), desc="Submitting eq2.sh") as pbar:
        while remaining:
