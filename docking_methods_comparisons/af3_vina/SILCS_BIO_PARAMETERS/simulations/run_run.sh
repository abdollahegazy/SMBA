...
#    EXISTING=$(ls "$simdir"/system_run[0-9][0-9][0-9].coor 2>/dev/null | wc -l)

#   if [ "$EXISTING" -lt 50 ]; then
#         echo ">>> Running job in $simdir ($EXISTING/50 files)"
#         (
#             cd "$simdir" && sbatch "$RUN_SCRIPT" 
#         ) 
#     else
#         echo ">>> Skipping $simdir (already has $EXISTING files)"
#     fi
